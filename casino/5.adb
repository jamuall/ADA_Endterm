...
procedure five is
   ...
   protected Manager is
      procedure AddPlayer(v : Integer);
      procedure AddPoint(v : Integer);
      procedure Print;
   private
      totalP : Integer := 0;
      totalPo: Integer := 0;
   ...
   protected body Manager is
      procedure AddPlayer(v : Integer) is
      begin 
         totalP := totalP + v;
      end AddPlayer;

      procedure AddPoint(v : Integer) is
      begin 
         totalPo := totalPo + v;
      end AddPoint;
      ...
      procedure Print is
      begin
         Printer.Print("Players from all Casinos : " & totalP'Image & " Points from all Players all Casinos : " & totalPo'Image);
      end Print;
   end Manager;
   ...
   begin
      ...
      while not played and trial < 2 and decide loop 
         select
               Casinos(ind).Enter;
               played := True;
               Printer.Print("Player" & name.all &": got in" & ind'Image);
